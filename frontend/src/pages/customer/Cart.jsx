...
export default function Cart() {
  ...
  const handleRemove = async (productId) => {
    // Optimistic: dispatch immediately, toast after
  ...
  if (items.length === 0 && !loading) {
    return (
      <>
        <Helmet><title>Shopping Cart — LocalMarket</title></Helmet>
        <div className="page-container py-8">
          <EmptyState
            type="cart"
            title="Your cart is empty"
            description="Looks like you haven't added anything yet. Explore our local products and start shopping!"
            action="/products"
  ...
  return (
    <>
      <Helmet><title>Cart ({item_count} items) — LocalMarket</title></Helmet>
      <div className="page-container py-8">
        <h1 className="font-display font-bold text-2xl text-gray-900 dark:text-white mb-8">
          Shopping Cart <span className="text-gray-400 font-normal text-lg">({item_count} items)</span>
        </h1>
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/products/${item.product_id}`} className="font-medium text-gray-900 dark:text-white hover:text-primary-600 line-clamp-2 text-sm transition-colors">
                      {item.title}
                    </Link>
                  ...
                  <div className="flex flex-col items-end gap-3">
                    <button onClick={() => handleRemove(item.product_id)} className="text-gray-400 hover:text-red-500 transition-colors p-1">
                      <FiTrash2 className="text-lg" />
                    </button>
                    <div className="flex items-center gap-1 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                      <button onClick={() => handleQtyChange(item.product_id, item.quantity - 1)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="card p-5">
              ...
              {coupon_code ? (
                ...
                <form onSubmit={handleApplyCoupon} className="flex gap-2">
                  <input type="text" value={coupon} onChange={e => setCoupon(e.target.value)} placeholder="Enter code..." className="input-field flex-1 py-2 text-sm" />
                  <button type="submit" disabled={applyingCoupon} className="btn-primary text-sm px-4 py-2">{applyingCoupon ? '...' : 'Apply'}</button>
                </form>
              )}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  <span>Shipping</span>
                  <span>{shippingFee === 0 ? <span className="text-green-600 font-medium">🎉 FREE</span> : formatCurrency(shippingFee)}</span>
                </div>
                {shippingFee > 0 && (
                  <p className="text-xs text-gray-400">Add ₹{(500 - subtotal).toFixed(0)} more for free delivery</p>
                )}
              ...
              <motion.button
                ...
                className="btn-primary w-full mt-5 py-3.5 flex items-center justify-center gap-2 text-base"
              >
                Proceed to Checkout →
              </motion.button>
              <Link to="/products" className="btn-outline w-full mt-2.5 text-center block text-sm py-2.5">
                Continue Shopping
              </Link>
            </div>
          </div>
